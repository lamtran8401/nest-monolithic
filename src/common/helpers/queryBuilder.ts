...
export function getQueryBuilder<Entity extends BaseEntity>(
  ...
  for (const orderKey in order) queryBuilder.orderBy(`entity.${orderKey}`, order[orderKey] as any);
...
function addWhere<Entity extends BaseEntity>(
  ...
  for (let i = 0; i < whereKeys.length; i++) {
    ...
    if (i === 0) whereMethod = 'where';

    const key = whereKeys[i];

    let queryString = `entity.${key} = ${where[key]}`;
    if (where[key] instanceof FindOperator && where[key]?._type === 'arrayContains') {
      queryString = `entity.${key} @> ARRAY[:members]::INTEGER[]`;
      queryBuilder[whereMethod](queryString, {
        members: where[key].value.toString(),
      });
...
function addQuery<Entity extends BaseEntity>(
  ...
  queryKeys.forEach(key => {
    ...
    if (whereKeys.length === 0) whereMethod = 'where';
    if (queryObjects.length > 1) queryString = `${key} LIKE '%${value}%'`;
